...
function HttpAPI(discovery, port, presets, notificationApi) {
  ...
  function handleAction(options, callback) {
    ...
    setInterval(function(){
      var currentState = getPlayerState(player);
      if (JSON.stringify(storedState) !== JSON.stringify(currentState)) {
        storedState = currentState;
        notificationApi.sendPlayerstateChangedNotification(currentState);
      }
    }, 100);
