...
function Discovery(log) {
  ...
  log.info("binding SSDP to port", port);
  ...
  for (var name in interfaces) {
    log.info('discovering all IPs from', name);
  ...
  for (var ip in sockets) {
    var socket = dgram.createSocket('udp4', function(buffer, rinfo) {
      ...
      if (!location) {
        log.error("location undefined", rinfo.address);
        return;
      }
    ...
    req.on('end', function () {
      ...
      saxParser.on('endNode', function (elem, attr, uq, tagend, getStrNode) {
        ...
        if (elem == "ZoneGroupState") {
          ...
          log.debug("FavoritesUpdateID", notifyState);
  ...
  function trySubscribe( deviceDescription, address ) {
    ...
    log.info("subscribing to topology", address);
    ...
    http.get(options, function (res) {
      ...
      log.info("using local endpoint", _this.localEndpoint);
  ...
  function subscribe(path) {
    ...
    var client = http.request({
      ...
      if (res.statusCode == 200) {
        ...
        log.error("subscribe failed!", sids[path], res.statusCode);
  ...
  this.aggregateGroupVolume = function (volumeData) {
    ...
    groupVolumeTimer = setTimeout(function () {
      log.debug('emitting group-volume');
  ...
  eventServer.on('error', function (e) {
    if (e.code == 'EADDRINUSE') {
      log.error("port in use", _this.notificationPort, "trying new one");
...
Discovery.prototype.applyPreset = function (preset) {
  ...
  if (!preset.players || preset.players.length == 0) {
    log.error("your preset doesn't contain any players");
    return;
  }
  ...
  if (coordinator.coordinator.uuid == coordinator.uuid && preset.players.length > 1) {
    log.info("skipping breakout because already coordinator");
    ...
    log.info("ungrouping", coordinator.roomName, "to prepare for grouping");
    asyncSeries.push(function (callback) {
  ...
  for (var i = 1; i < preset.players.length; i++) {
    ...
    if (!player) {
      log.error("invalid playerName", playerInfo.roomName);
      continue;
    }
...
Discovery.prototype.setToggleLED = function (enabled) {
  ...
  log.debug("update players with setToggleLED");
